...
const sortParticipants = (participants = []) => {
  ...
  participants.forEach((p, i) => {
    if (!p?.role) throw new ApiError(400, `participants[${i}].role is required`);
    if (!p?.userId) throw new ApiError(400, `participants[${i}].userId is required`);
  });
...
const validateRoom = ({ roomType, rideId, participants }) => {
  ...
  if (roomType === "RIDE") {
    if (!rideId) throw new ApiError(400,"rideId is required for ride chat");
    if (!(has(ROLES.USER) && has(ROLES.DRIVER))) {
      throw new ApiError(400,"Ride chat must be USER ↔ DRIVER only");
    }
    if (has(ROLES.ADMIN)) throw new ApiError(400,"Admin cannot join ride chat");
  }

  if (roomType === "SUPPORT") {
    if (!has(ROLES.ADMIN)) throw new ApiError(400,"Support chat must include admin");
    if (rideId) throw new ApiError(400,"Support chat does not need rideId");
  }
};

// ensures that room is created only once
export const findOrCreateRoom = async ({ roomType, rideId = null, participants }) => {
  if (!roomType) throw new ApiError(400,"roomType is required");
  if (!participants || participants.length < 2) throw new ApiError(400,"participants are required");
...
export const sendMessage = async ({ roomId, senderRole, senderId, text }) => {
  ...
  if (!sender) throw new ApiError(403, "You are not allowed to send message in this room");
  ...
  if (!receiver) throw new ApiError(400, "Receiver not found in this room");
